...
class TaskRepository extends ServiceEntityRepository
{
    ...
    /*tsi is Mialy's test */
    public function findByFiltersQuery(bool $isAdmin, string $searchTitle, int $minEstimate, int $maxEstimate)
    {
        $qb = $this->createQueryBuilder('t');
        if (!$isAdmin) {
            $qb->andWhere('t.isPublic = :isPublic')->setParameter('isPublic', true);
        }
        if (!empty($searchTitle)) {
            $qb->andWhere('t.title LIKE :searchTitle')->setParameter('searchTitle', '%' . $searchTitle . '%');
        }
        $qb->andWhere('t.estimate BETWEEN :min AND :max')
